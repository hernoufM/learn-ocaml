...
module Exercise = struct
  ...
  module Meta0 = struct
    ...
    let get id = Lazy.force !index >|= fun index -> Exercise.Index.find index id
  end
        ...
        (let tbl = Hashtbl.create 223 in
         ...
         | Unix.Unix_error (Unix.ENOENT, _, _) ->
             ...
             Exercise.Index.fold_exercises
               (fun () id _ -> Hashtbl.add tbl id (Exercise.Status.default id))
               () index;
             Lwt.return tbl
         | e -> Lwt.fail e)
  ...
  module Library = struct
    ...
    let ( / ) = Filename.concat
    let store_dir () = !static_dir / Learnocaml_index.server_libraries_dir

    let exec_read_words command =
      Lwt.catch
        (fun () ->
          Lwt_process.(with_process_full @@ shell command) @@ fun proc ->
          Lwt_io.read_line proc#stdout >>= fun line ->
          Lwt.return @@ String.split_on_char ' ' @@ String.trim line)
        (function _ -> Lwt.return [])

    let read_file path =
      Lwt_unix.file_exists path >>= fun exists ->
      if not exists then Lwt.fail_with ("file " ^ path ^ " doesn't exists")
      else Lwt_io.with_file ~mode:Lwt_io.Input path Lwt_io.read

    let bundle_file path (lib : t) =
      Lwt_io.(open_file ~mode:Output path) >>= fun oc ->
      Lwt_io.write_value ~flags:Marshal.[ No_sharing ] oc lib

    let unbundle_file path : t Lwt.t =
      Lwt_io.(open_file ~mode:Input path) >>= fun ic -> Lwt_io.read_value ic

    let init (meta, cmis) =
      Format.kprintf exec_read_words
        "ocamlfind query %s -predicates byte -format \"%%d %%a\"" meta
      >>= function
      | [ path; cma ] ->
          let name = Filename.remove_extension cma in
          Format.kprintf exec_read_words "ocamldep -sort -nocwd -I %s %s" path
            (path / "*.mli")
          >>= fun mlis ->
          let cmis =
            List.map
              (fun f -> Filename.basename f |> Filename.remove_extension)
              mlis
            |> List.filter (fun f -> List.mem f cmis)
          in
          Lwt.return { meta; name; path; cmis }
      | _ -> Lwt.fail_with "bad ocamlfind query format"

    let store { meta; path; name; cmis } =
      read_file @@ (path / name) ^ ".cma" >>= fun cma ->
      read_file @@ (path / name) ^ ".js" >>= fun js ->
      Lwt_list.map_p (fun f -> read_file @@ (path / f) ^ ".cmi") cmis
      >>= fun cmis ->
      bundle_file (store_dir () / name) { meta_name = meta; cma; js; cmis }

    let get { name; _ } =
      Lwt.catch
        (fun () -> unbundle_file @@ (store_dir () / name))
        (function _ -> Lwt.fail_with "no library found in server's directory")
        ...
        (let tbl = Hashtbl.create 223 in
         ...
         Lwt_stream.iter_s
           (fun f ->
             ...
             | "." | ".." -> Lwt.return_unit
             | f ->
                 unbundle_file (dir / f) >|= fun content ->
                 Hashtbl.add tbl content.meta_name content)
           (Lwt_unix.files_of_directory dir)
         >|= fun () -> tbl)

    let get_local id =
      Lazy.force tbl >>= fun tbl ->
      (try Hashtbl.find tbl id with Not_found -> failwith "No library found")
      |> Lwt.return  
  end
